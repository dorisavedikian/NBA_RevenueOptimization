...
def load_or_create_games():
        ...
        dim_games = pd.DataFrame(
            {
                ...
                "opponent": games["MATCHUP"]
                .str.extract(r"vs\. ([A-Z]{3})|@ ([A-Z]{3})")
                .bfill(axis=1)[0],
            }
        )
    else:
    ...
    dim_games["is_weekend"] = (
        dim_games["day_of_week"].isin(["Friday", "Saturday", "Sunday"]).astype(int)
    )
...
def create_promotions():
    return pd.DataFrame(
        [
            ...
            {
                "promotion_id": 1,
                "promotion_name": "Family Pack",
                "discount_percent": 0.15,
            },
            {
                "promotion_id": 2,
                "promotion_name": "Student Night",
                "discount_percent": 0.25,
            },
            {
                "promotion_id": 3,
                "promotion_name": "Military Discount",
    ...
    customers = pd.DataFrame(
        {
            ...
            "age_group": np.random.choice(
                age_groups, n_customers, p=[0.18, 0.32, 0.24, 0.16, 0.10]
            ),
            "income_bracket": np.random.choice(
                income_brackets, n_customers, p=[0.25, 0.35, 0.25, 0.15]
            ),
...
def generate_transactions(dim_games, dim_sections, dim_promotions, dim_customers):
    ...
    for _, game in dim_games.iterrows():
        demand_score = (
            0.50 + 0.35 * game["opponent_strength"] + 0.10 * game["is_weekend"]
        )
        ...
        for _, section in dim_sections.iterrows():
            ...
            while seats_to_sell > 0:
                ...
                promo = dim_promotions[
                    dim_promotions["promotion_id"] == promotion_id
                ].iloc[0]
                ...
                purchase_date = game["game_date"] - pd.Timedelta(
                    days=int(days_before_game)
                )
                ...
                transactions.append(
                    {
                        ...
                        "channel": np.random.choice(
                            channels, p=[0.45, 0.35, 0.15, 0.05]
                        ),
...
def generate_web_sessions(dim_games):
    ...
    for _, game in dim_games.iterrows():
        visitors = int(
            25000 + 60000 * game["opponent_strength"] + 7000 * game["is_weekend"]
        )

        for _ in range(visitors // 50):
            added_to_cart = np.random.binomial(
                1, 0.32 + 0.15 * game["opponent_strength"]
            )
            ...
            sessions.append(
                {
                    ...
                    "source": np.random.choice(
                        sources, p=[0.28, 0.24, 0.20, 0.18, 0.10]
                    ),
...
def main():
    ...
    fact_transactions = generate_transactions(
        dim_games, dim_sections, dim_promotions, dim_customers
    )
